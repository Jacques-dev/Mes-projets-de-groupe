...
<div class="row">
  <div class="col-lg-12">
    <form action="fonctions/ProjectQueries.php" method="post" id="createProject">
      Créer un projet<br>
      <input type="text" name="nomProjet" placeholder="Nom du projet"><br>
      <button type="submit" name="submitProject">Créer</button><br>
    </form>
  </div>
</div>
    ...
    <?php
      ...
      while($row = $result->fetch_assoc()) {
        ...
        <form action='fonctions/ProjectQueries.php' method='post'>
          <button class="custom-btn btn-2" type='submit' name='submitSelectProject' value=<?= $projectId; ?>><span><?= $row['nom']; ?></span></button>
          <!-- <button type='submit' name='submitSelectProject' value=<?= $projectId; ?> class='chooseProject'><?= $row['nom']; ?></button> -->
          <!-- <button type='submit' name='submitDeleteProject' value=<?= $projectId; ?> class='deleteProject'><i class='fas fa-trash'></i></button> -->
          <button class="btn-1" type='submit' name='submitDeleteProject' value=<?= $projectId; ?>>
            <div class="icon">
              <svg class="top">
                <use xlink:href="#top">
              </svg>
...
<div class="row">
  <div class="col-lg-12">
    ...
    <?php
      ...
      while($row = $result->fetch_assoc()) {
        $projectId = $row["id"];
        echo "<form action='fonctions/ProjectQueries.php' method='post'>";
        echo "<.btn-1 type='submit' name='submitSelectProject' value='$projectId' class='chooseProject'>".$row['nom']."</button>";
        echo "<button type='submit' name='submitDeleteSharedProject' value='$projectId' class='deleteProject'><i class='fas fa-trash'></i></button>";
        echo "</form>";
      }
    ?>
  </div>
